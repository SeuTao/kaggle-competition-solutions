...
from model.model_stnet import StNetV2, StNetV2_slim, \
                              StNetV2_slim_ChangePool,\
                              StNetV2_slim_ChangePool_Dblock, \
                              StNetV2_slim_Dblock, model50, model50_Dblock_SCSE, model101_Dblock_SCSE
...
class DecoderSolver(object):
    def __init__(self, config):
        ...
        self.is_non_empty = config.is_non_empty
    ...
    def build_model(self):
        ...
        elif self.model == 'stnet34_V2_slim_Dblock':
            self.G = StNetV2_slim_Dblock()

        elif self.model == 'model50':
            self.G = model50()

        elif self.model == 'model50_Dblock_SCSE':
            self.G = model50_Dblock_SCSE()

        elif self.model == 'model101_Dblock_SCSE':
            self.G = model101_Dblock_SCSE()
    ...
    def train_fold(self, fold_index):
        ...
        data_loader = get_5foldloader(self.image_size, self.batch_size, fold_index, mode= 'train', is_non_empty=self.is_non_empty)
        val_loader = get_5foldloader(self.image_size, self.batch_size, fold_index, mode = 'val', is_non_empty=self.is_non_empty)
...
def main(config):
    ...
    if config.mode == 'val_fold_NoTTA':
        solver = DecoderSolver(config)
        val_loader = get_5foldloader(config.image_size, config.batch_size, 0, mode = 'val', is_non_empty=config.is_non_empty)

        solver.val_NoTTA(config.train_fold_index, True, val_loader)

        # solver.pretrained_model = '20_final'
        # solver.val_NoTTA(config.train_fold_index, True, val_loader)
        # solver.pretrained_model = '22_final'
        # solver.val_NoTTA(config.train_fold_index, True, val_loader)
        # solver.pretrained_model = '24_final'
        # solver.val_NoTTA(config.train_fold_index, True, val_loader)
        # solver.pretrained_model = '26_final'
        # solver.val_NoTTA(config.train_fold_index, True, val_loader)
        # solver.pretrained_model = '28_final'
        # solver.val_NoTTA(config.train_fold_index, True, val_loader)
        # solver.pretrained_model = '30_final'
        # solver.val_NoTTA(config.train_fold_index, True, val_loader)
        # solver.pretrained_model = '22_final'
        # solver.val_NoTTA(config.train_fold_index, True, val_loader)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='model101_Dblock_SCSE')
    parser.add_argument('--fmap', type=int, default=32)
    parser.add_argument('--model_name', type=str, default='model101_Dblock_SCSE_resize768_non_empty')
    parser.add_argument('--image_size', type=int, default=768)
    parser.add_argument('--batch_size', type=int, default=1)
    parser.add_argument('--mode', type=str, default='train', choices=['train', 'val','val_fold','val_fold_NoTTA','test','test_fold'])

    parser.add_argument('--pretrained_model', type=str, default=r'10_final')
    ...
    parser.add_argument('--is_non_empty', type=bool, default=True)
    # Model hyper-parameters
    parser.add_argument('--num_epochs', type=int, default=80)
    parser.add_argument('--lr_steps', type=str, default='15,30')
